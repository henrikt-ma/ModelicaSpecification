...
The iteration variables are the implicitly declared iteration variable(s) if inside a \lstinline!for!-loop body, \cref{for-equations-repetitive-equation-structures} and \cref{for-statement}, or the body of a reduction expression, \cref{reduction-functions-and-operators}.
...
Outer component declarations shall not have the prefix \lstinline!input! and may only use \lstinline!output! in state machines with special semantics, see \cref{merging-variable-definitions}.
...
\begin{itemize}
\item
  If there are two (or more) \lstinline!outer! declarations with the same name and both lack matching \lstinline!inner! declarations, it is required that the \lstinline!outer! declarations are of the same class.
\item
  An \lstinline!outer! declaration's class must not be partial.
\item
...
\item
  The annotations defined in \cref{annotations-for-the-graphical-user-interface} does not affect this process, other than that \lstinline!missingInnerMessage! can be used for the diagnostic (and possibly error messages).
\end{itemize}
