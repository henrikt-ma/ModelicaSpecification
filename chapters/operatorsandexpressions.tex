...
\item
  In relations of the form \lstinline!v1 == v2! or \lstinline!v1 <> v2!, \lstinline!v1! or \lstinline!v2! shall, unless used in a function without \lstinline!GenerateEvents = true! (see \cref{modelica:GenerateEvents}), not be a subtype of \lstinline!Real!.
  \begin{nonnormative}
  ...
  Allowing the comparisons in functions without \lstinline!GenerateEvents = true! does not introduce any kind of function behavior that an external function would not be able to express anyway.
  Functions with \lstinline!GenerateEvents = true! can be treated as being directly inlined, and thus follow the same rules that apply outside functions.
  \end{nonnormative}
\item
...
In this document, the notation \lstinline!time.start! is used to refer to the value of \lstinline!time! when the simulation starts (i.e., during simulation initialization).
Note that \lstinline!time.start! is not a valid Modelica expression.
...
If either of the arguments is \lstinline!Real! the result is \lstinline!Real! otherwise \lstinline!Integer!.
\begin{nonnormative}
Note, outside of \lstinline!when!-clause bodies, state events are triggered when the return value changes discontinuously.
...
\end{lstlisting}
The expressions \lstinline!in0! and \lstinline!in1! shall be subtypes of a common type $T$, where $T$ is either \lstinline!Real!, \lstinline!Integer!, \lstinline!Boolean!, or an enumeration type.
Further, \lstinline!x! shall be a subtype of \lstinline!Real!, \lstinline!positiveVelocity! is a \lstinline!Boolean!, and \lstinline!initialPoints! and \lstinline!initialValues! are arrays of subtypes of \lstinline!Real! and $T$, respectively.
The two unnamed outputs assigned to \lstinline!out0! and \lstinline!out1! also have type $T$.
Regarding standard type coercion (\cref{standard-type-coercion}), $T$ is \lstinline!Real! if and only if at least one of \lstinline!in0!, \lstinline!in1! and \lstinline!initialValues! is a subtype of \lstinline!Real!.
...
When the \lstinline!spatialDistribution! call is discrete-time (determined independently of \lstinline!initialPoints! and \lstinline!initialValues!, see \cref{discrete-time-expressions}), two adjacent values in \lstinline!initialValues! may only differ where the associated \lstinline!initialPoints! values are equal.
...
\begin{nonnormative}
Since \lstinline!spatialDistribution! can only be used in equations where the outputs are assigned to variables, it is not possible to wrap the call in \lstinline!noEvent! unless $T$ is \lstinline!Real!, since only \lstinline!Real! variables may have non-discrete-time variability.
\end{nonnormative}
...
It is not valid as a general expression.
\end{nonnormative}
...
The $\mathit{expr}$ can be both an ordinary expression or the right-hand side of a statement or equation receiving the result of a function call with multiple outputs, see \cref{simple-equality-equations,assignments-from-called-functions-with-multiple-results}.
...
At event iterations after the first one at each event, during initialization, and during continuous integration the operator always returns \lstinline!false!.
At the start of the simulation the first event iteration occurs after the initialization even if the start argument is identical to the start time of the simulation.
...
\begin{nonnormative}
This can be applied to continuous-time variables in \lstinline!when!-clause bodies, see \cref{discrete-time-expressions} for the definition of discrete-time expression.
\end{nonnormative}
...
This evaluation sequence is called \firstuse{event iteration}\index{mixed system iteration|seealso{event iteration}} (compare \willintroduce{mixed system iteration} described below).
...
Since a \lstinline!when!-clause branch triggers when \lstinline!v and not pre(v)!, the implicit variable is considered wrapped in \lstinline!pre($\ldots$)! and becomes part of the event iteration convergence criterion.

\begin{nonnormative}
If \lstinline!v! and \lstinline!pre(v)! are only used in \lstinline!when!-clause bodies, the translator might mask event iteration for variable \lstinline!v! since \lstinline!v! cannot change during event iteration.
...
A common -- but not standardized -- approach to solving these systems of equations is to apply a fix point iteration scheme, alternating between solving the \lstinline!Real! and the other variables.
This sort of evaluation sequence is often referred to as \firstuse{mixed system iteration}\index{event iteration|seealso{mixed system iteration}} (compare \emph{event iteration} defined above).
...
\end{lstlisting}\end{synopsis}
\begin{semantics}
Re-initializes $x$ with $\mathit{expr}$.
Only allowed in a \lstinline!when!-equation body, meaning that re-initialization will only occur at events.
...
There are five levels of variability of expressions, starting from the least variable:
\begin{itemize}
\item
  constant variability
\item
  evaluable variability
\item
...
\item
  ...
  \item
    \lstinline!Connections.uniqueRootIndices(entry,exit,message)!
  \item
...
\item
  Expressions in a \lstinline!when!-clause body, \lstinline!initial equation!, or \lstinline!initial algorithm!.
\item
...
\item
  Unless inside \lstinline!noEvent!: \lstinline!spatialDistribution($\mathit{in0}$, $\mathit{in1}$, $x$, $\mathit{positiveVelocity}$, $\ldots$)!, if $\mathit{in0}$, $\mathit{in1}$, and $\mathit{positiveVelocity}$ are discrete-time expressions.
\item
...
The following rule applies to \lstinline!while!-statements and \lstinline!for!-clauses inside an \lstinline!if!-clause body, except when also inside a \lstinline!when!-clause body.
When such a \lstinline!while!-statement or \lstinline!for!-clause is controlled by a non-discrete-time (that is continuous-time, but not discrete-time) switching expression, it is not legal to contain assignments to discrete-time variables, equations between discrete-time expressions, or real elementary relations/functions that should generate events.
