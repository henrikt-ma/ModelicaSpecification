...
The corresponding component of \lstinline!R! in \lstinline!M! must not be a conditional component.
...
While it cannot be expressed that a function is infinitely differentiable, something like \lstinline!smoothOrder=100! gives tools sufficient guarantees about smoothness for virtually all practical purposes.
It is recommended to specifically use 100 to make it easy to recognize.
...
If {\lstinline!zeroDerivative!} can be used, that is preferable over {\lstinline!noDerivative!} as the assumptions are clearly stated and checked.
...
Some requirements only apply when the C mapping of a record class is needed.
The C mapping is only needed when the record appears in the input and output types of some \lstinline!external!-clause.

The \lstinline!Include! annotation (providing the struct definition, see \cref{modelica:Include}) is required when the C mapping of the record class is needed.
A deprecated semantics is that the \lstinline!Include! annotation may be omitted.
In this case, the struct is assumed to have standard padding and the same members in the same order as the record class, and the \lstinline!CStructTag! and \lstinline!CName! annotations apply as usual.
...
{\lstinline!T[$\mathit{dim}_{1}$, $\ldots$, $\mathit{dim}_{n}$]!}
                      & \emph{Not allowed}                    & \emph{Not allowed} \\
...
\end{lstlisting}\end{synopsis}
\begin{semantics}
Allowed for \lstinline!external!-clauses and record class annotations (\cref{annotations-for-c-mapping-of-records}).
The included code should be valid C89 code.

When \lstinline!annotation(Include="insertedCode")! appears in a record class annotation, the \lstinline!"insertedCode"! shall provide a definition (not merely a declaration without members) of the C struct to which the Modelica record is mapped.
The struct members may be in a different order compared to the record components, additional members may exist, and padding may differ.
The \lstinline!"insertedCode"! content shall be possible to insert where include directives are typically placed in generated code.
See \cref{modelica:CStructTag} and \cref{modelica:CName} regarding the struct tag and the member names.

\begin{nonnormative}
For record mapped to the C struct \lstinline[language=C]!myfoo!, examples of \lstinline!"insertedCode"! include:
\begin{itemize}
\item
  An \lstinline[language=C]!#include! directive including a file (header or source) with a declaration of the struct \lstinline[language=C]!myfoo!.
...
\item
  A piece of C code directly defining the struct \lstinline[language=C]!myfoo!.
\end{itemize}
\end{nonnormative}

The following applies to \lstinline!Include! annotations appearing in \lstinline!external!-clauses.
...
\subsection{Annotations for C Mapping of Records}\label{annotations-for-c-mapping-of-records}

Record classes may have the following annotations to support the mapping to C defined in \cref{records}.
\begin{center}
\begin{tabular}{l|l l}
\hline
\tablehead{Annotation} & \tablehead{Description} & \tablehead{Details}\\
\hline
\hline
{\lstinline!CStructTag!} & Name of struct in C & \Cref{modelica:CStructTag}\\
{\lstinline!CName!} & Name of struct member in C & \Cref{modelica:CName}\\
{\lstinline!Include!} & Include directives & \Cref{modelica:Include}\\
{\lstinline!IncludeDirectory!} & Directories containing header files & \Cref{modelica:IncludeDirectory}\\
\hline
\end{tabular}
...
The \lstinline!Include! and \lstinline!IncludeDirectory! annotations are also used with similar semantics for external functions.

\begin{annotationdefinition}[CStructTag]
\begin{synopsis}\begin{lstlisting}
/*literal*/ constant String CStructTag
\end{lstlisting}\end{synopsis}
\begin{semantics}
Allowed for record class annotations.

The annotation \lstinline!CStructTag = "c_tag"! specifies that \lstinline[language=C]!c_tag! is the struct tag in the C mapping of the Modelica record class.
By default, the struct tag is the same as the record class name.
A default struct tag which is not a valid in C is allowed, provided that the the C mapping of the record class is not needed.

If multiple record classes with \lstinline!CStructTag! annotation have the same \lstinline!c_tag!, the records must be type compatible.
\end{semantics}
\end{annotationdefinition}

\begin{annotationdefinition}[CName]
\begin{synopsis}\begin{lstlisting}
/*literal*/ constant String CName
\end{lstlisting}\end{synopsis}
\begin{semantics}
Allowed for component annotations inside record classes.

The annotation \lstinline!CName = "c_name"! specifies that the name of the C struct member corresponding to the record component is \lstinline[language=C]!c_name!.
By default, the struct member name is the same as the record component name.
A default struct member name which is not a valid in C is allowed, provided that the the C mapping of the record class is not needed.
\end{semantics}
\end{annotationdefinition}
...
\subsubsection{Record Mapping}\label{record-mapping-example}

\begin{example}
The following external function illustrates the annotations in \cref{annotations-for-c-mapping-of-records}.
It assumes that the external code uses \lstinline!MyComp_! as a prefix for declarations to avoid namespace collisions.
\begin{lstlisting}[language=modelica]
record R
  Real x;
  Integer y;
  // No Include annotation (deprecated semantics).
  annotation(CStructTag="MyComp_R");
end R;
...
record R2
  ...
  annotation(Include="
struct MyComp_R2 {
  double a,w,z; // Different order, and a is unused.
};"
  );
end R2;
...
function foo
  ...
  output R2 r2;
external "C"
  MyComp_foo(r, r2) annotation(
      Include="void MyComp_foo(const struct MyComp_R*r, struct MyComp_R2*r2);"
    );
end foo;
\end{lstlisting}
