...
The annotations listed below define various forms of documentation for a class.
They are all sub-annotations of the \fmtannotationindex{Documentation} annotation.
\begin{center}
\begin{tabular}{l|l l}
\hline
...
\hline
\hline
{\lstinline!Documentation.info!} & Class presentation & \Cref{modelica:Documentation.info}\\
{\lstinline!Documentation.revisions!} & Class revision history & \Cref{modelica:Documentation.revisions}\\
{\lstinline!Documentation.styleSheets!} & Style sheets for documentation & \Cref{modelica:Documentation.styleSheets}\\
{\lstinline!Documentation.figures!} & Simulation result figures & \Cref{modelica:Documentation.figures}\\
\hline
\end{tabular}
\end{center}
...
This section describes some of the sub-annotations of the \lstinline!Documentation! annotation.

\begin{annotationdefinition*}[info]\label{modelica:Documentation.info}\annotationindex{info}\index{Documentation@\robustinline{Documentation}!\robustinline{info} sub-annotation}
\begin{synopsis}\begin{lstlisting}
record Documentation
  /*literal*/ constant String info = "";
  $\ldots$
end Documentation;
\end{lstlisting}\end{synopsis}
\begin{semantics}
The \lstinline!info! annotation gives a textual description of the class.
...
\begin{annotationdefinition*}[revisions]\label{modelica:Documentation.revisions}\annotationindex{revisions}\index{Documentation@\robustinline{Documentation}!\robustinline{revisions} sub-annotation}
\begin{synopsis}\begin{lstlisting}
record Documentation
  /*literal*/ constant String revisions = "";
  $\ldots$
end Documentation;
\end{lstlisting}\end{synopsis}
\begin{semantics}
The \lstinline!revisions! annotation gives a revision history of the class.

The string is interpreted in the same way as in the \lstinline!info! annotation, see \cref{modelica:Documentation.info}.
...
\begin{annotationdefinition*}[styleSheets]\label{modelica:Documentation.styleSheets}\annotationindex{styleSheets}\index{Documentation@\robustinline{Documentation}!\robustinline{styleSheets} sub-annotation}
\begin{synopsis}\begin{lstlisting}
record Documentation
  /*literal*/ constant String[:] styleSheets;
  $\ldots$
end Documentation;
\end{lstlisting}\end{synopsis}
\begin{semantics}
Only considered for top-level packages, and applies to the entire package.
...
Each element of the \lstinline!styleSheets! annotation array specifies a cascading style sheet.
...
This section describes the \lstinline!figures! sub-annotation of the \lstinline!Documentation! annotation.

\begin{annotationdefinition*}[figures]\label{modelica:Documentation.figures}\annotationindex{figures}\index{Documentation@\robustinline{Documentation}!\robustinline{figures} sub-annotation}
\begin{synopsis}\begin{lstlisting}
record Documentation
  Figure[:] figures;
  $\ldots$
...
\end{lstlisting}\end{synopsis}
\begin{semantics}
Each element of the \lstinline!figures! annotation array defines a simulation result figure.
A figure is a graphical container that can contain several plots.
Details about the \lstinline!Figure! record are given in \cref{figure-plot-properties}.
\end{semantics}
\end{annotationdefinition*}

\subsubsection{Figure and Plot Properties}\label{figure-plot-properties}

Each figure in the \lstinline!figures! annotation is described by a \fmtannotationindex{Figure}:
\begin{lstlisting}[language=modelica]
record Figure
...
A plot inside a \lstinline!Figure! is described by a \fmtannotationindex{Plot}:
\begin{lstlisting}[language=modelica]
record Plot
  /*literal*/ constant String title "Title meant for display";
  /*literal*/ constant String identifier = "" "Identifier meant for programmatic access";
...
A \lstinline!Plot! \lstinline!identifier! on the other hand is only required to be unique among the \lstinline!plots! in the same \lstinline!Figure! annotation.
...
For example, when \lstinline!base! is 10, major axis ticks should preferably be placed at integer powers of 10, and natural alternatives that a tool may use for major axis tick labels could look like $0.001$ or $10^{-3}$.
...
A numeric value can be embedded in the same way, for example \lstinline!%{100'ms'}!, and shall be displayed consistently with a final variable value.
...
In \lstinline!%{$\mathit{variable}$}!, the $\mathit{variable}$ has the form of \productionref{result-reference}.
The tool should consider the \lstinline!displayUnit!-attribute (if any) when selecting the unit in which to display a $\mathit{variable}$.
In \lstinline!%{$\mathit{value}$}!, the $\mathit{value}$ has the form of \productionref{unsigned-number} (i.e., a number optionally followed by a unit, see \cref{unitful-literals}) optionally preceded by a minus sign.

Text markup escape sequences don't apply inside the $\mathit{variable}$ or $\mathit{value}$.
This means that a complete \productionref{result-reference} or numeric value shall be scanned before looking for the terminating closing brace.
...
Vendor-specific markup can be added to a variable replacement in the form \lstinline!%$\mathit{vendorSpecificMarkup}${$\mathit{variable}$}!, and similarly for an embedded value.
...
This is due to the current inability to define a base URI referencing the current figure.
...
The annotations listed below define the graphical representation of a class.
\begin{center}
\begin{tabular}{l|l l}
\hline
\tablehead{Annotation} & \tablehead{Description} & \tablehead{Details}\\
\hline
\hline
{\lstinline!Icon!} & Icon view of a class & \Cref{modelica:Icon}\\
{\lstinline!Diagram!} & Diagram view of a class & \Cref{modelica:Diagram}\\
{\lstinline!Placement!} & Instance placement in icon or diagram & \Cref{modelica:Placement}\\
\hline
\end{tabular}
\end{center}

The graphical representation of a class consists of two abstraction layers: an icon layer and a diagram layer.
The icon typically visualizes the component by hiding hierarchical details, while the diagram layer typically shows the hierarchical decomposition using icons of subcomponents and lines representing connections.

\begin{annotationdefinition}[Icon]
\begin{synopsis}\begin{lstlisting}
record Icon
...
\begin{semantics}
Allowed for class annotations.
The \lstinline{Icon} annotation defines the icon view of the class.

The \lstinline!graphics! is specified as an ordered sequence of graphical primitives described in \cref{common-definitions}.
...
\begin{annotationdefinition}[Diagram]
\begin{synopsis}\begin{lstlisting}
record Diagram
  CoordinateSystem coordinateSystem;
  GraphicItem[:] graphics;
...
\begin{semantics}
Allowed for class annotations.
The \lstinline{Diagram} annotation defines the diagram view of the class.

The \lstinline{coordinateSystem} and \lstinline{graphics} serve the same purpose as in the \lstinline!Icon! annotation, see \cref{modelica:Icon}.
\end{semantics}
\end{annotationdefinition}
...
The \lstinline!interaction! attribue specifies interactive features, see \cref{user-input}.
...
All gradients below use pure white for \lstinline!fillColor! and pure black for \lstinline!lineColor!.
...
If the rectangle would have had rounded corners (by specifying a positive \lstinline!radius!), the geometry of the gradient fill would not change, meaning that the \lstinline!lineColor! would not have been attained.
...
\begin{figure}[H]
  \begin{center}
% Warning: Is there really no need for a workaround here, similar to the one in \cref{fig:cylinder-gradient}?
% Image for smooth = None:
%   \includegraphics{sphere-gradient-triangle}
% Image for smooth = Bezier:
    \includegraphics{sphere-gradient-bezier}
  \end{center}
  \caption{
    Applying \lstinline!FillPattern.Sphere! to a \lstinline!Polygon! with \lstinline!smooth = Bezier!.
...
Visibility and placement is controlled using the \lstinline!Placement! annotation.

\begin{annotationdefinition}[Placement]
\begin{synopsis}\begin{lstlisting}
record Placement
  Boolean visible = true;
...
The \lstinline{Placement} annotation defines the placement of the component's graphics as part of the graphics of the enclosing class.

The visibility of the component in the diagram layer of the enclosing class is defined by \lstinline!visible!.
For a connector component, the component's diagram layer defines the content to be displayed, while the icon layer is used for other component kinds.

\begin{nonnormative}
The use of a connector's diagram layer facilitates opening up a hierarchical connector to allow connections to its internal subconnectors.
\end{nonnormative}

The \lstinline!transformation! defines the placement in the diagram layer of the enclosing class.
The \lstinline!Transformation! type is defined below.

The \lstinline!iconVisible! only applies to public connector components, and defines the visibility of the component in the icon layer of the enclosing class (protected connectors are never visible in icons).
Sub-connectors in a hierarchical connector are only visible if they can be connected to.
The connector's icon layer defines the content to be displayed.
The default for \lstinline!iconVisible! is to be the same as \lstinline!visible!.

The \lstinline!iconTransformation! only applies to public connector components, and defines the placement of the \lstinline!Icon! annotation graphics in the icon layer of the enclosing class.
The default for \lstinline!iconTransformation! is to be the same as \lstinline!transformation!.
\end{semantics}
\end{annotationdefinition}
...
It defines a coordinate system transformation by applying the attributes in the order \lstinline!extent!, \lstinline!rotation!, \lstinline!origin!, as follows:
\begin{enumerate}
\item
...
It has the following definition (it is not equal to the \lstinline!Text! primitive as part of graphics -- the differences are marked with comments):
\begin{lstlisting}[language=modelica]
record Text
  extends GraphicItem;
  Extent extent;
...
The \lstinline!index! is one of the points of the \lstinline!Line! (numbered 1, 2, 3, \ldots{}, where negative numbers count from the end, thus -1 indicates the last one).
The \lstinline!origin! (used to interpret \lstinline!extent! and \lstinline!rotation!) is specified relative to the this point on the \lstinline!Line!.
...
The meanings of \lstinline!extent! and \lstinline!rotation! (inherited from \lstinline!GraphicItem!), as well as \lstinline!fontSize!, \lstinline!fontName! \lstinline!textStyle!, and \lstinline!textColor! are the same as for the other \lstinline!Text! annotation in \cref{text}.

When \lstinline!horizontalAlignment! is \lstinline!TextAlignment.Automatic!, the tool automatically determines the alignment to reduce collisions with graphical items.
...
\item
  \lstinline!%name! replaced by the name of the component (i.e., the identifier for it in the enclosing class).
\item
  \lstinline[deletekeywords={[2]class}]!%class! replaced by the name of the class (only the last part of the hierarchical name).
\item
  \lstinline!%{$\mathit{par}$}! and \lstinline!%$\mathit{par}$! replaced by the value of the parameter or variable with a binding or modifier setting the value $\mathit{par}$.
  The tool should consider the \lstinline!displayUnit!-attribute of $\mathit{par}$ (if any) when selecting the unit in which to display the value.
  A displayed value with unit shall be formatted according to the BIPM specification.
  E.g., with
\begin{lstlisting}[language=modelica]
parameter Real t(unit = "s", displayUnit = "ms") = 0.1
\end{lstlisting}
  it would be natural for a tool to display \lstinline!%{t}! as \emph{100 ms}.
  The intent is that the text is easily readable, thus if $\mathit{par}$ is of an enumeration type, replace \lstinline!%{$\mathit{par}$}! by the item name, not by the full name.
  \begin{example}
  If \lstinline!par = "Modelica.Blocks.Types.Enumeration.Periodic"!, then \lstinline!%{par}! should be displayed as \emph{Periodic}.
  \end{example}
  When quoted identifiers (e.g., \lstinline!'quoted ident'! or \lstinline!'}'!) or composite names (i.e., not simple identifiers) are involved, the form \lstinline!%{$\mathit{par}$}! must be used.
  Here, $\mathit{par}$ is a general \productionref{component-reference}, and \lstinline!%{a.p}! gives the value of the parameter \lstinline!p! in the component \lstinline!a!.
...
\item
  \lstinline!%{$\mathit{value}$}! replaced by $\mathit{value}$ displayed in the same style as a parameter or variable in the item above.
  Here, $\mathit{value}$ has the form of \productionref{unsigned-number} (i.e., a number optionally followed by a unit, see \cref{unitful-literals}) optionally preceded by a minus sign.
  Unlike \lstinline!%$\mathit{par}$!, there is no form without braces.
  \begin{nonnormative}
  Compared to writing the number and unit directly in the text, this makes the presentation consistent with that of parameter values in several ways: the minus sign of a negative value, formatting and precision of the number, separation between number and unit, and the presentation of the unit.
  \end{nonnormative}
\end{itemize}
...
The \lstinline!horizontalAlignment! must not be \lstinline!TextAlignment.Automatic! within this \lstinline!Text! annotation (compare \cref{connections1}).
...
The annotations in this section define features for using the diagram layer to interact with a running simulation.

\begin{annotationdefinition}[interaction]
\begin{synopsis}\begin{lstlisting}
InteractionItem[:] interaction;
...
\end{lstlisting}%
\annotationindex{InteractionItem}%
\end{synopsis}
\begin{semantics}
Allowed for components and class annotations, and also as an attribute of a \lstinline!GraphicItem!.

Each element of the \lstinline!interaction! array specifies how some variable should be modified.
Similar to tunable parameters (see \cref{parameters}), the actions of modifying variables through these annotations deviate from the mathematical formalism given in \cref{modelica-dae-representation}.
Also, the critera for allowing a variable to be modified, as well as the semantics of the modification, are tool-depndent.

\begin{nonnormative}
Examples of variables that might be possible to modify include states, variables assigned in \lstinline!when!-clauses, and the so-called tunable parameters.
\end{nonnormative}
\end{semantics}
\end{annotationdefinition}

The various item definitions extending from \lstinline!InteractionItem! are defined below.
In all cases, the variable to be modified is given by an attribute of the appropriate type for the value modification in question.
These attribute modifications must have the form of a component reference.
...
\begin{lstlisting}[language=modelica]
record OnMouseMoveYSetReal extends InteractionItem
  Real yVariable "Variable to change when cursor moved in y direction";
  Real minValue;
  Real maxValue;
...
When converting between units, the unit offset must be ignored for relative quantities.
Examples of unit conversion include plotting a results in a given unit (see \lstinline!Axis! in \cref{figure-plot-properties}), editing of parameters, and implicit unit conversion of unitful literals (see \cref{unitful-literals}).
...
The reason could be that the mechanism contains a planar loop
or that joints constrain the same motion.
For planar loops, use one RevolutePlanarLoopConstraint per loop."
  );
end Frame;
\end{lstlisting}
\end{example}
...
Supported file formats are the same as for the \lstinline!Bitmap! graphical item defined in \cref{bitmap}.
...
The old and new modifiers shall be component references, except for the \lstinline!cardinality! cases listed below.
...
\item
  The file $\mathit{packageName}$\filename{\ }$\mathit{packageVersion}$\filename{.mo} (one space character separating $\mathit{packageName}$ from $\mathit{packageVersion}$)\\
...
\item
  The directory $\mathit{packageName}$\filename{\ }$\mathit{packageVersion}$ with the file \filename{package.mo} directly inside it (one space character separating $\mathit{packageName}$ from $\mathit{packageVersion}$)\\
...
// Requires license features:
//    ("LicenseOption1" and "LicenseOption2")
//    or "LicenseOption3"
...
package MyLibraryAuthorization_Tool
  annotation(Authorization(
    ...
    license = {
      License(
        licensee = "Organization B, Mr.X",
        id = {"lic:1269"}), // tool license number
      License(
        licensee = "Organization C, Mr. Y",
        id = {"lic:511"},
        expirationDate="2010-06-30",
        operations = {"ExportBinary"}),
      License(
        licensee="Organization D, Mr. Z",
...
The annotation listed below allows a top-level class to specify license conditions.
\begin{center}
\begin{tabular}{l|l l}
\hline
\tablehead{Annotation} & \tablehead{Description} & \tablehead{Details}\\
\hline
\hline
{\lstinline!License!} & License files & \Cref{modelica:top-License}\\
\hline
\end{tabular}
\end{center}

\begin{annotationdefinition*}[License]\label{modelica:top-License}\index{License@\robustinline{License}!top-level class annotation}
\begin{synopsis}\begin{lstlisting}
...
External functions may also contain this annotation (with similar content), see \cref{modelica:extfun-License}.
